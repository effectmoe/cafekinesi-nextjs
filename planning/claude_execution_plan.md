...
- [x] **暗黙的 any の解消 / モジュール不足対応**:
  - Batch 1-3 で 162件 → 97件 (-65件) まで削減。
  - 主な修正: Radix UI/shadcnパッケージ追加、`revalidatePath`引数修正。
- [x] **残存エラー対応 (Part 2)**:
  - 未使用コンポーネント削除 (`chart.tsx`, `calendar.tsx` 等)。
  - `tsconfig.json` の `exclude` に `scripts/`, `studio/` を追加。
  - **結果**: 残留エラー 21件 (目標達成)。ビルド成功。

## 3. 完了サマリー
Next.js 16 へのアップグレードおよび徹底的な型エラー修正が完了しました。
- **ブランチ**: `develop`
- **エラー削減**: 162件 → 21件 (-87%)
- **状態**: ビルド成功、Lint通過、Vercelデプロイ済み。
- **次のアクション**: ユーザーによる実機動作確認（地図、Twitter埋め込み）。
