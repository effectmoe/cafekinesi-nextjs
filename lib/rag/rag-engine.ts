...
${isComparisonQuery ? '- 「最も」「一番」などの比較を求められた場合は、【イベント価格一覧（昇順）】を参照し、リストの順序通りに正確に答えてください' : ''}
${isComparisonQuery ? '- 「最も安い」場合は一覧の**1番目**、「最も高い」場合は一覧の**最後**を選んでください' : ''}
    ...
    if (vectorResults.length > 0) {
      ...
      if (events.length > 0) {
        ...
        eventPrices.forEach((e: any, idx: number) => {
          context += `${idx + 1}. ${e.title}: ¥${e.price.toLocaleString()}\n`;
        });
      }
    }
