...
- **推測や想像で答えず、コンテキストに基づいた正確な情報のみを提供してください**
  ...
  private filterEventsByConditions(query: string, results: any[]): { filtered: any[], filterInfo: string } {
    ...
    // 特定のイベント名が質問に含まれているか確認
    const hasSpecificEventName = events.some((e: any) => {
      const titleMatch = e.content.match(/イベント[：:]\s*([^\n]+)/);
      const title = titleMatch ? titleMatch[1].trim() : '';
      // イベント名の一部でもマッチすればOK（「ハッピーオーラス」「ピーチタッチ」など）
      return title && query.includes(title);
    });

    // ステータスフィルタ（特定のイベント名が指定されている場合は適用しない）
    // 例: 「ハッピーオーラス入門の予約は可能ですか？」→ フィルタしない（満席でも結果に含める）
    // 例: 「予約できるイベントはありますか？」→ フィルタする（受付中のみ）
    if (isOpenQuery && !hasSpecificEventName) {
